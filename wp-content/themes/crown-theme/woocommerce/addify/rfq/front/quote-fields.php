...
<table class="quote-fields">
    ...
    foreach ( $quote_fields as $key => $field ) :
        ...
        if ( ! apply_filters( 'afrfq_display_field_input', true, $field_id ) ) {
            continue;
        }
        ...
        $is_readonly = in_array( $afrfq_field_label, $readonly_inputs )  ? 'readonly' : '';

        if ( 'terms_cond' == $afrfq_field_type ) {
            $dynamic_class = ! empty( $afrfq_field_quote_types ) ? ' afrfq-dynamic-field' : '';
            ?>
            <tr class="addify-option-field<?php echo esc_attr( $dynamic_class ); ?>">
        ...
        <tr class="addify-option-field<?php echo esc_attr( $dynamic_class ); ?>">
            ...
            <td>
                ...
                switch ( $afrfq_field_type ) {
                    case 'text':
                        ?>
                        <input type="text" placeholder="<?php echo esc_html( $afrfq_field_placeholder ); ?>" name="<?php echo esc_html( $afrfq_field_name ); ?>" value="<?php echo esc_html( $field_data ); ?>" <?php echo esc_attr( $required ); ?> <?php echo esc_attr( $is_readonly ); ?> >
                        <?php
                        break;
                    case 'time':
                        ?>
