...
export default function Edit( { attributes, className, isSelected, setAttributes } ) {
	...
	const TEMPLATE = [
		[ 'core/heading', { placeholder: 'Page Header Title...', level: 1 } ], // defaults to H1 tag for header block
		[ 'core/paragraph', {} ]
	];
	...
	return [
		...
		<InspectorControls key="inspector-controls">
			...
			<PanelBody title={ 'Background Image' } className={ 'crown-blocks-background-image' } initialOpen={ false }>
				...
				{ !! enableGradientOverlay && <ColorPicker
					label="Gradient Start Color"
					color={gradientColorStart}
					onChangeComplete={ (value) => { setAttributes({ gradientColorStart: value }); } }
					enableAlpha
				/> }
				...
				{ !! enableGradientOverlay && <ColorPicker
					...
					onChangeComplete={ (value) => { setAttributes({ gradientColorEnd: value }); } }
					enableAlpha
				/> }

				{ !! enableGradientOverlay && <SelectControl
					options={[
			...
			<PanelBody title={ 'Enable Bottom Gradient' } initialOpen={ false }>
			...
			{ restrictContentWidth && <PanelBody title={ 'Content Alignment' } initialOpen={ false }>

				<BaseControl label="Horizontal">
					<ButtonGroup>
				...
				<BaseControl label="Vertical">
					<ButtonGroup>
						<Button isPrimary={ contentAlignVertical == 'top' } isSecondary={ contentAlignVertical != 'top' } onClick={ (e) => setAttributes({ contentAlignVertical: 'top' }) }>Top</Button>
						<Button isPrimary={ contentAlignVertical == 'center' } isSecondary={ contentAlignVertical != 'center' } onClick={ (e) => setAttributes({ contentAlignVertical: 'center' }) }>Center</Button>
						<Button isPrimary={ contentAlignVertical == 'bottom' } isSecondary={ contentAlignVertical != 'bottom' } onClick={ (e) => setAttributes({ contentAlignVertical: 'bottom' }) }>Bottom</Button>
					</ButtonGroup>
				</BaseControl>
		...
		<div { ...useBlockProps( customProps ) }>
			<div className="section-bg">
				{ backgroundImageUrl && <div className={ 'bg-image' } style={ {
					...
					{ enableGradientOverlay && gradientColorStart && gradientColorEnd && <div className="gradient-overlay" style={ backgroundGradientStyles }></div> }
				</div> }
