<?php

if(!class_exists('Crown_Block_News_Index')) {
	class Crown_Block_News_Index extends Crown_Block {

		public static $name = 'news-index';

		public static function init() {
			parent::init();
			add_action( 'wp_enqueue_scripts', array( __CLASS__, 'register_dependency_scripts' ), 100 );
			add_action( 'wp_enqueue_scripts', array( __CLASS__, 'localize_script_data' ) );
		}

		public static function register_dependency_scripts() {
			$dependency_scripts = array( 'jquery' );
			global $wp_scripts;
			$block_script = $wp_scripts->query( 'crown-blocks-' . self::$name . '-script', 'registered' );
			if ( $block_script ) {
				foreach ( $dependency_scripts as $dep_handle ) {
					if( ! in_array( $dep_handle, $block_script->deps ) && $wp_scripts->query( $dep_handle, 'registered' ) ) {
						$block_script->deps[] = $dep_handle;
					}
				}
			}
		}

		public static function localize_script_data() {
			$data = array(
				'baseUrl' => get_home_url(),
				'blockClassName' => 'wp-block-crown-blocks-' . self::$name
			);
			wp_localize_script( 'crown-blocks-' . self::$name . '-script', 'crownBlockNewsIndexData', $data );
		}

		public static function get_attributes() {
			return array(
				'className' => array( 'type' => 'string', 'default' => '' )
			);
		}

		public static function render( $atts, $content ) {

			$newsroom_term = get_term_by( 'slug', 'newsroom', 'category' );

			$filter_args = array(
				array(
					'name' => 'n_category',
					'type' => 'taxonomy-term-radios',
					'taxonomy' => 'category',
					'sort' => 'menu_order_ASC',
					'all_label' => 'All'
				)
			);
			if ( $newsroom_term ) {
				$filter_args[] = array(
					'name' => 'n_sub_category_' . $newsroom_term->term_id,
					'type' => 'taxonomy-term-select',
					'taxonomy' => 'category',
					'parent' => $newsroom_term->term_id,
					'all_label' => 'All Newsroom'
				);
			}
			$filters = self::get_post_feed_block_filters( $filter_args );

			$query_args = array(
				'post_type' => array( 'post' ),
				'posts_per_page' => 8
			);
			$query = self::get_post_feed_block_filtered_query( $query_args, $filters );

			$block_args = array(
				'layout' => 'simple',
				'pagination' => 'infinite'
			);

			ob_start();
			self::render_post_feed_block( $query, $filters, $atts, $block_args );
			return ob_get_clean();
		}


		protected static function render_post_feed_block( $query, $filters, $atts, $args = array() ) {

			?>
				<div class="wp-block-crown-blocks-news-index-wrap">
					<?php parent::render_post_feed_block( $query, $filters, $atts, $args ); ?>
					<div class="block-sidebar">
						<div class="inner">
							<h3>CSA Social</h3>
							<h4>Connect with Us!</h4>
							<?php echo do_shortcode( '[custom-twitter-feeds]' ); ?>
						</div>
					</div>
				</div>
			<?php

		}


		public static function render_post_feed_article() {
			?>
				<article <?php post_class(); ?>>
					<a href="<?php the_permalink(); ?>" target="<?php echo apply_filters( 'post_link_target', '_self' ); ?>">
						<div class="inner">

							<div class="bg">
								<?php echo wp_get_attachment_image( get_post_thumbnail_id(), 'large' ); ?>
							</div>

							<div class="contents">

								<?php $categories = get_the_terms( get_the_ID(), 'category' ); ?>
								<?php $category_names = ! empty( $categories ) ? array_map( function( $n ) { return $n->name; }, $categories ) : array(); ?>
								<?php if ( ! empty( $category_names ) ) { ?>
									<h6 class="entry-categories"><?php echo implode( ', ', $category_names ); ?></h6>
								<?php } ?>

								<h3 class="entry-title"><?php the_title(); ?></h3>

								<p class="entry-date"><?php the_time( 'n/j/Y' ); ?></p>

								<?php $source = get_post_meta( get_the_ID(), 'post_source', true ); ?>
								<?php if ( ! empty( $source ) ) { ?>
									<p class="entry-source">From: <?php echo $source; ?></p>
								<?php } ?>

							</div>
							
							<?php $cta = get_post_meta( get_the_ID(), 'post_cta_label', true ); ?>
							<p class="cta"><span class="btn btn--link link-arrow btn--link-blue"><span class="btn-label"><?php echo ! empty( $cta ) ? $cta : 'Read More'; ?></span><span class="btn__arrow"></span></span></p>

						</div>
					</a>
				</article>
			<?php
		}


	}
	// Crown_Block_News_Index::init();
}
