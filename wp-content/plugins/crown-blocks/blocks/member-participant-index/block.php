...
if(!class_exists('Crown_Block_Member_Participant_Index')) {
	class Crown_Block_Member_Participant_Index extends Crown_Block {

		public static $name = 'member-participant-index';
		...
		public static function localize_script_data() {
			...
			wp_localize_script( 'crown-blocks-' . self::$name . '-script', 'crownBlockMemberParticipantIndexData', $data );
		}
		...
		public static function render( $atts, $content ) {
			...
			if ( empty( $atts['viewAllLinkUrl'] ) ) {
				$filters_args[] = array( 'name' => 'mpa_keywords', 'type' => 'search', 'expandable' => false, 'placeholder' => 'Search' );
			}
			...
			$query = self::get_post_feed_block_filtered_query( array(
				...
				'order' => 'ASC',
				'tax_query' => array(
					array( 'taxonomy' => 'member_level', 'field' => 'slug', 'terms' => 'participants' )
				)
			), $filters );
	...
	// Crown_Block_Member_Participant_Index::init();
}
