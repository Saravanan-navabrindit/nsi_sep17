(function($) {
	...
	var crownBlockData = crownBlockCatalogIndexData;
	...
	var submitForm = function(e) { $(this).closest('form').trigger('submit'); };
	...
	var updateBlockPostFeed = function(block, url) {
		...
		if((!$('.pagination-wrapper.infinite', block).length || !url.match(/\/page\/\d+\//)) && block.offset().top < $(window).scrollTop()) {
			...
			window.scrollTo({ top: block.offset().top - offset, behavior: 'smooth' });
		}
		$.ajax({
			url: url,
			blockId: blockId,
	...
	$(document).on('click', '.' + crownBlockData.blockClassName + ' form.post-feed-filters a.reset', function(e) {
		...
		$('.filter', form).each(function(i, el) {
			...
			if(defaultValue !== null) {
				...
				$('select option', filter).filter(function(i, el2) { return defaultValue.includes($(el2).val()); }).prop('selected', true);
				$('input[type=checkbox], input[type=radio]', filter).filter(function(i, el2) { return defaultValue.includes($(el2).val()); }).prop('checked', true);
			}
		});
	...
	$(document).on('click', '.' + crownBlockData.blockClassName + ' form.post-feed-filters .filter-title', function(e) {
		...
		fieldsContainer.css({ height: fieldsContainer.height() });
		filter.toggleClass('expanded');
		setTimeout(function() {
			if(!filter.hasClass('expanded')) {
				fieldsContainer.css({ height: 0 });
			} else {
				fieldsContainer.css({ height: $('> .inner', fieldsContainer).outerHeight() });
				setTimeout(function() { fieldsContainer.css({ height: 'auto' }); }, 200);
			}
		}, 0);
	});
	...
	var updateSubFilters = function(e) {
		$('.' + crownBlockData.blockClassName).each(function(i, el) {
			...
			var termId = $('.filter.name-n_category input:checked', form).first().val();
			if(termId && $('.filter.name-n_sub_category_' + termId, form).length) {
				block.addClass('sub-filter-active');
				$('.filter.name-n_sub_category_' + termId, form).addClass('active');
			} else {
				block.removeClass('sub-filter-active');
