<?php
/**
 * Class AF_Email_Register_New_Account file.
 *
...
if ( ! class_exists( 'AF_Email_Register_New_Account', false ) ) :
	 ...
	 *
	 * @class       AF_Email_Register_New_Account
	 * @version     3.5.0
	 * @package     WooCommerce\Classes\Emails
	 * @extends     WC_Email
	 */
	class AF_Email_Register_New_Account extends WC_Email {
		...
		public function __construct() {
			$this->id             = 'af_email_register_new_account';
			$this->customer_email = true;
			$this->title          = __( 'Addify New account/Pending User', 'af_custom_fields' );
			$this->description    = __( 'Customer "new account" emails are sent to the customer when a customer signs up.', 'af_custom_fields' );
			$this->template_base  = AF_CF_PLUGIN_DIR . 'templates/';
			$this->template_html  = 'emails/register-new-account.php';
			$this->template_plain = 'emails/plain/register-new-account.php';
		...
		public function trigger( $user_id, $user_pass = '', $password_generated = false ) {
			...
			if ( $user_id ) {
				...
				$this->placeholders = array(
					...
					'{customer-details}' => AF_C_F_Emails::get_customer_details( $user_id, 'registration' ), 
		...
		public function get_email_content() {
			...
			return apply_filters( 'addify_email_content_' . $this->id, $this->format_string( get_option('af_c_f_pending_approval_email_text') ), $this->object, $this );
