...
$billing_fields = get_option( 'af_checkout_billing_fields' );

?>
<h3><?php esc_html_e('Billing fields by user roles', 'af_custom_fields'); ?></h3>
...
<div class="af_cf_accordion">
	...
	foreach ( $roles as $role_key => $role_name ) { 
		...
		<div class="af_ac_role_container" data-role_key="<?php echo esc_html( $role_key ); ?>">
			<span class="click-to-select hidden"><?php echo esc_html__('Click to select', 'af_custom_fields'); ?></span>
			<span class="selected hidden"><?php echo esc_html__('Selected', 'af_custom_fields'); ?></span>
			<span class="drag-to-sort hidden"><?php echo esc_html__('Drag to Sort', 'af_custom_fields'); ?></span>
			<span class="click-to-remove hidden"><?php echo esc_html__('Click to Remove', 'af_custom_fields'); ?></span>
			<table class="draggable">
				<tr class="heading" >
					<th colspan="2">
						<?php echo esc_html__('All Billing Fields', 'af_custom_fields'); ?>
					</th>
				</tr>
				<?php 
			...
			<table class="droppable">
				...
				foreach ( $selected_fields as $field_key ) {
					$field_label = $fields[ $field_key ]['label'];
					?>
					<tr class="<?php echo esc_attr( $field_key ); ?>" data-field_key="<?php echo esc_attr( $field_key ); ?>" >
						<td class="checkbox_col">
						...
						<td class="field_name">
							<input type="hidden" class="hidden_field" value="<?php echo esc_html( $field_key ); ?>" name="af_checkout_billing_fields[<?php echo esc_html( $role_key ); ?>][]">
							<p class="field_name"><?php echo esc_attr( $field_label ); ?></p>
						</td>
						<td class="remove">
