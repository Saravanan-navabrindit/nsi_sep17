		...
		<thead>
			<tr>
				...
				<?php if ( $of_price_display ) : ?>
					<th class="product-price"><?php esc_html_e( 'Requested Price', 'addify_rfq' ); ?></th>
				<?php endif; ?>
					...
					<tr class="woocommerce-cart-form__quote-item <?php echo esc_attr( apply_filters( 'addify_quote_item_class', 'cart_item', $quote_item, $quote_item_key ) ); ?>">
						...
						<?php if ( $price_display ) : ?>
							<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'addify_rfq' ); ?>">
								<?php
									...
									if ( $min_qty > 1 && preg_match( '/>(\d+(?:\.\d+)?)/', $price_html, $matches ) ) {
										$product_price = wc_get_price_excluding_tax( $_product );
										$price_html = preg_replace( '/>\d+(\.\d+)?/', '>' . number_format( $product_price * $min_qty, 2 ), $price_html );
									}
									...
									echo '<small style="display: block; opacity: .6; line-height: 1.2;">' . ( $min_qty <= 1 ? 'Each' : 'per ' . $min_qty ) . '</small>';
								?>
							</td>
						<?php endif; ?>
						...
						<?php if ( $of_price_display ) : ?>
							<td class="product-price offered-price" data-title="<?php esc_attr_e( 'Offered Price', 'addify_rfq' ); ?>">
								<input type="number" min="0.01" class="input-text offered-price-input text" step="any" name="offered_price[<?php echo esc_attr( $quote_item_key ); ?>]" value="<?php echo esc_attr( ! empty( $offered_price ) ? $offered_price : '' ); ?>">
							</td>
						<?php endif; ?>	
						...
						<?php if ( $of_price_display ) : ?>
							<td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'addify_rfq' ); ?>">
								<?php
									...
									echo wp_kses_post( apply_filters( 'addify_quote_item_subtotal', wc_price( $offered_price * $quote_item['quantity'] / $min_qty ), $quote_item, $quote_item_key ) ); // phpcs:ignore WordPress.Security.EscapeOutput
