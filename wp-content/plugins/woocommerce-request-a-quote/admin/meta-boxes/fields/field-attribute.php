...
$default_values = array(
	...
	'billing_state'       => __( 'Billing State', 'addify_rfq' ),
	...
	'shipping_state'      => __( 'Shipping State', 'addify_rfq' ),
...
<div class="afrfq-metabox-fields">
    ...
    <th>
        <div class="option-head">
            <h3>
                <?php echo esc_html__( 'Quote Types', 'addify_rfq' ); ?>
            </h3>
        </div>
    </th>
    <td>
		...
		if(is_object($addify_rfq) && is_object($addify_rfq->quote_types_obj)) {
            ...
            ?>
            <select class="multi-select afrfq-quote-types-select" name="afrfq_field_quote_types[]" multiple id="afrfq_field_quote_types">
                <?php foreach ( $quote_types as $quote_type ) : ?>
                    <option value="<?php echo intval( $quote_type->ID ); ?>" <?php echo in_array( $quote_type->ID, array_map( 'intval', $afrfq_field_quote_types ) , true ) ? 'selected="selected"' : ''; ?> title="<?php echo esc_attr(get_the_title( $quote_type )); ?>">
                        <?php echo esc_html( get_the_title( $quote_type ) ); ?>
                    </option>
                
				 <?php endforeach; ?>
            </select>
           
            <p class="description"><?php echo esc_html__( 'Select the quote types to associate with this field.', 'addify_rfq' ); ?></p>
					<?php
				} else {
					echo esc_html("Please ensure the 'AF_R_F_Q_Quote_Types' class and object have been properly instantiated in your base file to fetch all quote types");
				}
                ?>
            </td>
        </tr>
