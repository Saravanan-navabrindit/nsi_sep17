...
class AF_R_F_Q_Quote_Fields {
	...
	public function get_field_default_value( $field_id, $user_id ) {
		...
		// 	$country = (string) get_user_meta( (int) $user_id, 'shipping_country', true );

		// 	if ( isset( WC()->countries->states[ $country ] ) && isset( WC()->countries->states[ $country ][ $data ] ) ) {
		// 		return strtolower( WC()->countries->states[ $country ][ $data ] );
		// 	}
		// }
		...
		if ( empty( intval( $user_id ) ) ) {
			...
			if ( !empty( $billling_field ) ) {
				...
				if ( !empty( $field_value ) && is_email( $field_value ) ) {
					return apply_filters( 'afrfq_get_user_email', $field_value, $quote_id, $billing );
				}
			...
			if ( is_object( $user ) ) {
				...
				if ( !$billing ) {
					...
					return apply_filters( 'afrfq_get_user_email', $user->user_email, $quote_id, $billing );
				...
				if ( !empty( get_user_meta( $user_id, 'billing_email', true ) ) ) {
					...
					return apply_filters( 'afrfq_get_user_email',  get_user_meta( $user_id, 'billing_email', true ), $quote_id, $billing );
				} 
	...
	public function afrfq_validate_fields_data( $data ) {
		...
		foreach ( $fields as $field_post ) {
			...
			$field = $field_post->ID;
     	    ...
     	    $afrfq_field_quote_types = (array) get_post_meta( $field, 'afrfq_field_quote_types', true );
